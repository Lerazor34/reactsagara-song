import { LinkPagination } from "./models";
...
export function PaginationHelpers(
  {count , limit , page, maxLeftPagination = 3, maxRightPagination = 10} : props
)  {
  const paginations: Array<LinkPagination> = [];
  
  
  const maxPagination: number = Math.floor(count / limit) + (count % limit ? 1 : 0)
  const maxPageNumber = maxPagination < maxRightPagination ? maxPagination : maxRightPagination
  let pageNumber = 1;
  ...
  do {
    ...
    if(page_type === 'left'){
      if(pageNumber <= 6 ){
        ...
        if (maxPagination) {
          let currentpagenumber = maxPagination - (10 - pageNumber);
      ...
      if (pageNumber < 3) {
        ...
        let currentposition = 5 - pageNumber;
        let pagecurrent = page - currentposition;
    ...
    console.log(page_type,label,page,pageSkipStart,pageNumber,active);
