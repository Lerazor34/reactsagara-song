...
const getUser =() =>{
  ...
  const lsValue: string | null = localStorage.getItem(USER_LOCAL_STORGE_KEY)
  if (!lsValue) {
    return
  }
...
const PrivilegesValidation = ({path , currentUser, auth} : roleprops)  =>{
  ...
  if (admin && admin.length > 0) {
    ...
    privileges = privileges?.filter((p) => {
      let matching = new RegExp(p.uri.toString().toLowerCase());
        return (
          p.uri.toString().toLowerCase() ===
...
export {getAuth, setAuth, removeAuth, removeUser,setUser, getUser, PrivilegesValidation , AUTH_LOCAL_STORAGE_KEY, USER_LOCAL_STORGE_KEY}
