...
export async function hardDelete<Type>(collection: string, id: ID): Promise<ResponseModel<Type>> {
  return new Promise(async (resolve, reject) => {
    try {
      const response: ResponseModel<Type> = await ServiceHardDelete(collection, id)
      return resolve(response)
    } catch (e) {
      return reject(e)
    }
