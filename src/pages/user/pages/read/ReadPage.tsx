...
const ReadPage: FC = () => {
  ...
  const getRoleName = async (role: string[] | undefined) => {
    ...
    const [roleResult] = response.data.data as unknown as Array<RoleModel>;
    return roleResult?.name || defaultRole;
  };

  useEffect(() => {
    const getUser = async () => {
      const response = await getOneById(Collection, id);
      const responseUser: UserResponse = response.data as any;
